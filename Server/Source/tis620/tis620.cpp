...
int tis620::decode_utf8(const unsigned char* utf8, int len, unsigned int* codepoint) {
    ...
    if ((first & 0xE0) == 0xC0) {
        if (len < 2 || (utf8[1] & 0xC0) != 0x80) return -1;
    ...
    if ((first & 0xF0) == 0xE0) {
        if (len < 3 || (utf8[1] & 0xC0) != 0x80 || (utf8[2] & 0xC0) != 0x80) return -1;
    ...
    if ((first & 0xF8) == 0xF0) {
        if (len < 4 || (utf8[1] & 0xC0) != 0x80 || (utf8[2] & 0xC0) != 0x80 || (utf8[3] & 0xC0) != 0x80) return -1;
